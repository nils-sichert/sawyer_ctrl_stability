...
class impedance_ctrl:
    def __init__(self, robot_chain, nrOfJoints):
        ...
        self._nrOfJoints = nrOfJoints
        self._J  = None # 6x7
        self._J_1 = np.zeros((6,7)) # 6x7
    ...
    def run_impedance_controll(self, q, dq, pose, pose_desi, sampling_Time, J, g, M, C, Dd=np.identity(6), Kd=np.identity(6)):
        # q and dq are in KDL format
        self._update_Dd(Dd)
        self._update_Kd(Kd)
        self._update_J(q, J)
        # M = self._update_M(q)
        ...
        ddx = self._calc_ddx(pose, sampling_Time)
    ...
    def _calc_derr(self,dt):
        derr = np.array((1/dt)*(self._get_err()-self._get_err_1()))
    ...
    def _calc_ddx(self, pose, dt):
        ...
        ddx = np.array((1/dt)*(self.dpose_1-self.dpose)) # ddx
    ...
    def _set_J(self, q, J):
        ...
        # J = kdl.ChainJntToJacSolver(self._robot_chain) # Dimension is 6x7 matrix
        return J
    def _set_A(self):
